...
# why divide by 2? only using 16-bits of 32-bit samples
SDCARD_SAMPLE_BUFFER_SIZE_IN_BYTES = MIC_SAMPLE_BUFFER_SIZE_IN_BYTES // 2
NUM_SAMPLE_BYTES_TO_WRITE = RECORD_TIME_IN_SECONDS * \
...
def create_wav_header(sampleRate, bitsPerSample, num_channels, num_samples):
    ...
    o += (sampleRate * num_channels * bitsPerSample //
          8).to_bytes(4, 'little')  # (4byte)
    o += (num_channels * bitsPerSample // 8).to_bytes(2,
                                                      # (2byte)
                                                      'little')
    # (2byte)
    ...
    try:
        ...
        if num_bytes_read_from_mic > 0:
            ...
            num_bytes_snipped = snip_16_mono(
                mic_samples_mv[:num_bytes_read_from_mic], wav_samples_mv)
            num_bytes_to_write = min(
                num_bytes_snipped, NUM_SAMPLE_BYTES_TO_WRITE - num_sample_bytes_written_to_wav)
